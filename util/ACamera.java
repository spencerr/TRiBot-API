...
public class ACamera
{
    ...
    public boolean quickTurnToTile(Positionable tile) {
        scripts.api.util.Camera.quickFocus(tile);

        /*int optimalAngle = adjustAngleToTile(tile);
        int optimalRotation = Camera.getTileAngle(tile);

        ROTATION_METHOD method = Camera.getRotationMethod();
        Camera.setRotationMethod(ROTATION_METHOD.ONLY_MOUSE);
        rotationThread.rotating = true;
        angleThread.turning = true;



        if(Math.abs(optimalAngle - Camera.getCameraAngle()) > ANGLE_THRESHOLD)
            setCameraAngle(optimalAngle);

        if(Math.abs(optimalRotation - Camera.getCameraRotation()) > ROTATION_THRESHOLD)
            setCameraRotation(optimalRotation);

        long startTime = Timing.currentTimeMillis();

        while ((rotationThread.rotating || angleThread.turning) && Timing.currentTimeMillis() - startTime < 5000) {
            General.sleep(20, 50);
        }

        Camera.setRotationMethod(method);
    ...
    public boolean turnToTile(Positionable tile)
    {
        int optimalAngle = adjustAngleToTile(tile);
        int optimalRotation = Camera.getTileAngle(tile);
