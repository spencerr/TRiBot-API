...
public final class DirectedGraph<T> implements Iterable<T> {
    /* A map from banches in the graph to sets of outgoing edges.  Each
     * set of edges is represented by a map from edges to doubles.
     */
    public final Map<T, Map<T, Double>> mGraph = new HashMap<T, Map<T, Double>>();
     ...
     * @throws NoSuchElementException If either the start or destination banches
     *                                do not exist.
     */
    public void addEdge(T start, T dest, double length) {
        /* Confirm both endpoints exist. */
        if (!mGraph.containsKey(start) || !mGraph.containsKey(dest))
            return;//throw new NoSuchElementException("Both banches must be in the graph.");
    ...
    public void removeEdge(T start, T dest) {
        /* Confirm both endpoints exist. */
        if (!mGraph.containsKey(start) || !mGraph.containsKey(dest))
            throw new NoSuchElementException("Both banches must be in the graph.");
    ...
    /**
     * Returns an iterator that can traverse the banches in the graph.
     *
     * @return An iterator that traverses the banches in the graph.
     */
